...
def parse_args():
   ...
   #     choices=MODEL_TYPES,
    ...
    parser.add_argument("--inference_set_ratio", type=float, default=0.2, 
                        help="Ratio of training data to be used as inference set")
    parser.add_argument("--privacy_lambda", type=float, default=0.00001,
                        help="Lambda parameter for privacy regularization")
    parser.add_argument("--privacy_alpha", type=float, default=1.0,
                        help="Alpha parameter for privacy regularization")
...
def main():
    ...
    # # Add new arguments for privacy-preserving fine-tuning
    ...
    # # Parse the new arguments (without overriding existing args)
    # privacy_args, _ = parser.parse_known_args()
    # args.inference_set_ratio = privacy_args.inference_set_ratio
    # args.privacy_lambda = privacy_args.privacy_lambda
    # args.privacy_alpha = privacy_args.privacy_alpha
    
    random.seed(args.seed)
    
    # [... existing code until dataset preparation ...]
...
#        logger.warning("You are instantiating a new config instance from scratch.")
    ...
    # Modify the dataset split to include an inference set
    ...
    # [... existing code until group_texts function ...]
    ...
    # Split training data into train and inference sets
    train_full = lm_datasets["train"]
    eval_dataset = lm_datasets["validation"]
    
    # Create train and inference sets
    train_size = int(len(train_full) * (1 - args.inference_set_ratio))
    train_indices = random.sample(range(len(train_full)), train_size)
    inference_indices = [i for i in range(len(train_full)) if i not in train_indices]
    
    train_dataset = torch.utils.data.Subset(train_full, train_indices)
    inference_dataset = torch.utils.data.Subset(train_full, inference_indices)
    
    if accelerator.is_local_main_process:
        print(f"Training set size: {len(train_dataset)}")
        print(f"Inference set size: {len(inference_dataset)}")
    ...
    inference_dataloader = DataLoader(
        inference_dataset, shuffle=True, collate_fn=default_data_collator, batch_size=args.per_device_train_batch_size
    )
    eval_dataloader = DataLoader(
        eval_dataset, collate_fn=default_data_collator, batch_size=args.per_device_eval_batch_size
    )

    # [... existing code for model setup ...]
    ...
    model, optimizer, train_dataloader, inference_dataloader, eval_dataloader = accelerator.prepare(
        model, optimizer, train_dataloader, inference_dataloader, eval_dataloader
    )
    
    # [... existing code until training loop ...]
...
# Here freezing of the model except the last layer i.e the head is performed
    ...
    # Modified training loop with privacy-preserving fine-tuning
    for epoch in range(args.num_train_epochs):
        model.train()
        if accelerator.is_local_main_process:
            print(f"training epoch {epoch}")
            
        # Create iterators for train and inference dataloaders
        inference_iter = iter(inference_dataloader)
        cummulated_grads = {}
        
        for step, member_batch in enumerate(train_dataloader):
            # Reset gradients
            optimizer.zero_grad()
            
            # Get non-member batch (inference set batch)
            try:
                nonmember_batch = next(inference_iter)
            except StopIteration:
                inference_iter = iter(inference_dataloader)
                nonmember_batch = next(inference_iter)
            
            # Forward pass for member data (training set)
            nonmember_outputs = model(**nonmember_batch)
            nonmember_loss = nonmember_outputs.loss
            
            # Backward pass for member data to get member gradients
            accelerator.backward(nonmember_loss)
            
            # Store member gradients
            nonmember_grads = {}
            for name, param in model.named_parameters():
                if param.requires_grad and param.grad is not None:
                    nonmember_grads[name] = param.grad.clone()
            
            # Zero gradients before non-member pass
            optimizer.zero_grad()
            
            # Forward pass for non-member data (inference set)
            member_outputs = model(**member_batch)
            member_loss = member_outputs.loss
            
            # Backward pass for non-member data to get non-member gradients
            accelerator.backward(member_loss)
            # Store member gradients
            member_grads = {}
            for name, param in model.named_parameters():
                if param.requires_grad and param.grad is not None:
                    member_grads[name] = param.grad.clone()
            
            
            # Calculate privacy sensitivity and apply regularization
            module_sensitivities = {}
            
            # # First pass: calculate sensitivities and group by module
            # for name, param in model.named_modules():
            #     if param.requires_grad and param.grad is not None:
            #         # Calculate privacy sensitivity as the absolute difference between member and non-member gradients
            #         privacy_sensitivity = torch.abs(nonmember_grads[name] - param.grad)
                    
            #         # Group sensitivities by module (using the first part of the parameter name)
            #         module_name = name.split('.')[0]
            #         if module_name not in module_sensitivities:
            #             module_sensitivities[module_name] = []
            #         module_sensitivities[module_name].append(privacy_sensitivity.mean().item())
            
            # # Calculate mean sensitivity for each module
            # module_mean_sensitivities = {
            #     module: sum(sensitivities) / len(sensitivities)
            #     for module, sensitivities in module_sensitivities.items()
            # }
            
            # Second pass: apply regularization with sensitivity-based strength
            for name, param in model.named_parameters():
                if param.requires_grad and param.grad is not None:
                    # Get member gradient for this parameter
                    # param.grad = member_grads[name]
                    
                    # Calculate privacy sensitivity
                    privacy_sensitivity = torch.abs(member_grads[name] - nonmember_grads[name])
                    
                    # Get module name and mean sensitivity
                    module_name = name.split('.')[0]
                    # mean_sensitivity = module_mean_sensitivities[module_name]
                    
                    # Calculate regularization strength based on relative sensitivity
                    # if mean_sensitivity > 0:
                    relative_sensitivity = privacy_sensitivity / privacy_sensitivity.mean()
                    reg_strength = args.privacy_lambda * (relative_sensitivity ** args.privacy_alpha)
                    # else:
                    #     reg_strength = args.privacy_lambda
                    
                    # Apply L1 regularization with adaptive strength
                    l1_grad = reg_strength * torch.sign(param.data)
                    param.grad = member_grads[name] + l1_grad
                    if name not in cummulated_grads:
                        cummulated_grads[name] = torch.zeros_like(param.grad)
                    cummulated_grads[name] += param.grad.clone()
            ...
            if step % args.gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                for name, param in model.named_parameters():
                    if param.requires_grad and param.grad is not None:
                        param.grad = cummulated_grads[name]
                optimizer.step()
                lr_scheduler.step()
                for name, param in model.named_parameters():
                    if param.requires_grad and param.grad is not None:
                        cummulated_grads[name]=0
                optimizer.zero_grad()
                completed_steps += 1
